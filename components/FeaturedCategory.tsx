...
const FeaturedCategory = () => {
  return (
    <section className="min-h-screen">
      ...
      <p className="text-xs md:text-sm mt-2 font-medium text-slate-500 pl-5">Explore Our World: Dive into Diverse Learning Categories at Nepal Information Hub!</p>
      <div className="grid items-center space-y-4 py-16 md:grid-cols-2 md:gap-6 md:space-y-0 lg:grid-cols-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <Link
            href={"/"}
            key={i}
            className="relative aspect-[16/9]  w-auto rounded-md md:aspect-auto md:h-[200px] group hover:scale-105 duration-300"
          >
            <Image
